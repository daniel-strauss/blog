...
⚠️ �� �� �� �� �� �� *By reading this blog post, you will find out why this emoji sequence is here.*
...
Imagine you lived $40\cdot n$ years in the future and that the population had doubled every $40$ years. For simplicity, assume that in the current present, just one person is alive. Consequently, $2^n$ people live at your point in the future. You live far in the future, and $n$ is very large: $n=2^{10^{15}}$. "Today" is your friend's birthday party, and you do not have a present.
...
(TODO add screenshoooot)
In this screenshot, you see the control prompt for Mistral 7B and its text completion.
...
The inferred bounds can also be used to make statements about the training data probability space if a certain assumption about that training data probability space was made. Which? The training data was generated by a random neural network of a given architecture. We will evaluate the pros and cons of such an assumption in [Discussion of their Assumptions](#discussion-assumptions). Once the distribution of distributions (basically the Bayesian prior) that generates the data is formalized, we can make formal statements about the optimal (optimal with respect to a chosen loss function) estimator of the training data distribution. We call this estimator the optimal Bayesian estimator (OBE). If, additionally, one assumes that a well-trained transformer acts similarly to the OBE, one can infer from the theoretical performance of the Bayesian estimator the performance of the transformer. When making these assumptions, one has to be aware of certain dangers:
  - Is the Bayesian prior distribution - the assumed distribution of distributions - plausible? More on that in [Discussion of their Assumptions](#discussion-assumptions)
...
We say the distributions of $\lbrace D_1,...D_{M+1}\rbrace$ can be described by autoregressive models (as are transformers) that are parametrized by the random vectors $\lbrace \theta_1,...,\theta_m \rbrace$. We pack, for practicality, all parameter vectors into one $\theta := \lbrace \theta_1,...,\theta_m \rbrace$.
...
We know $\mathbb H(X) = \mathbb H(X|Y) + \mathbb I(X;Y)$, therefore it suffices to show that $\mathbb L_{M,T} \cdot MT = \mathbb H(H_T^{(M)})$. This is true since:
...
This equation means, roughly speaking, that the estimation error consists of these parts in $\theta$ that are conveyed to $H_T^{(M)}$. For example, as we often model $\theta$ as a continuous random variable and $H_T^{(M)}$ as a discrete random variable, $H_T^{(M)}$ cannot contain all information in $\theta$.
...
The authors separate the term into two parts: the meta-estimation error and the intra-document estimation error. The meta-estimation error describes the error of learning information shared by all documents. The intra-document estimation error is the error of learning the parameters of individual documents after having learned the shared information $\psi$. Let's say we have arbitrarily many training samples $A$ for our optimal Bayesian estimator in which $D_m$ does not occur. Since there are arbitrarily many of them, let's suppose $\psi$ has been discovered well enough such that $\mathbb I(D_m;\psi | A)$ is about 0. Then the error of estimating $D_m$ is about the intra-document error. This means that even if we perfectly train our model, there will be some error greater than zero related to the estimation of the probability that generates the data. Or will it really be greater than zero? More on this cliffhanger later.
...
To define the transformer architecture they define
  ...
  - $A^{(l)} \in \mathbb R^{r \times r}$ to be the product of key and query matrices of layer $l$
...
## Discussion of their Assumptions {#discussion-assumptions}
...
#### Lemma A: 
Let $m_1 = (P_1,V)$ and $m_2 = (P_2,V)$ be not dd but irreducible and finite Markov chains, $P_1 \neq P_2$ and $p_1 \in (0,1)$. 
...
To increase readability we denote: $p_{1|H_a} := \mathbb P[M=m_1|H_{a:t}]$ and arrive at:
...
As there are only finitely many possibilities for $h_k$, from B) it follows that there must exist $\epsilon > 0$, such that $p_{1|h_K} \in [0+\epsilon, 1-\epsilon]$. From this fact and A), we can conclude the sufficient statement on the right-hand side of a).
...
We construct $P_m$ like this: given $h \in \Sigma^{[K+i]}$, $x \in \Sigma$ and $f(h)(x) = p$, $P_m(h, s_{K+i-1}(h) \circ x)) = p$. If a transition $(a,b) \in V_m$ has not been defined in the previous sentence (e.g. $s_{K+i-1}(a) \neq p_{K+i-1}(b)$), then its probability is zero: $P_m((a,b)) = 0$. Note that by construction $f$ can be fully expressed in terms of $m$:
...
Let's suppose $T$ could express the probability distribution of this sequence, e.g. $\forall_{t \in \mathbb N} T(H_t)(X_{t+1}) = \mathbb P(X_t|H_t)$.
a) Then for $t=K_t$, $T$ implements the function $T(H_K)(X_{K_t+1}) = \mathbb P(X_{K_t + 1}|H_K)$.  
b) As $T$ has context length $K_t$, it must hold that $T(h)(x) = T(s_{K_t}(h))(x)$.
...
As transformers can express any distribution within our frame, and if we start to sample them in the most random way (e.g. highest entropy), should we then get the most random distribution of distributions (e.g. the distribution of distributions, such that we cannot find a Bayesian prior such that the optimal Bayesian estimator achieves a higher error)? Can we find what the uniform distribution is for finite discrete spaces, for AG probability distributions?
...
Assuming the training data was generated by a full autoregressive model with random parameters can be a good assumption to simplify further mathematical analysis, and maybe it is not too far off, in order to produce results with well-meaning and close bounds. Still, choosing random transformers as a full autoregressive model may be a bit arbitrary, and I think that further justification would be helpful to add validity.
...
be a full autoregressive model. As $A_f$ is surjective, there is a constant $\theta_0 \in \mathbb R^n$ such that $A_f(\theta_0) = A(\theta)$ and thus $H_T \sim A_f(\theta_0)$.
...
where $f^{-1}(\theta)_{i:j}$ defines the word that goes from the $i$'th character of the word $f^{-1}(\theta)$ until the $j$'th character.
I am sorry that this expression looks confusing, but I will try to explain: $f^{-1}(\theta)$ is just an infinitely long word. $A(\theta)(h)(x)$ returns 1 iff the word $h \circ x$ is a prefix of $f^{-1}(\theta)$. If $h$ is a prefix of $f(\theta)$ but $h \circ x$ is not, then $A(\theta)(h)(x) = 0$, and otherwise $A(\theta)$ is undefined. $A$ will start to make more sense once $\theta$ is a random variable.
