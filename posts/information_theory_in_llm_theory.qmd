...
- The authors called for further analysis of how a transformer can implement a sparse mixture of transformers. In this blog post, we show that a transformer cannot implement a sparse mixture of transformers.
- Furthermore, we show that any discrete autoregressive distribution can be modeled such that the estimation error equals any desired fraction of the sequence entropy — in particular zero or the full entropy. We use this to argue that the estimation error reflects properties of the chosen parameterization more than intrinsic learning difficulty. The according results have been written more cleanly in this [pdf-file](/data/Formal_Writeup_Blogpost_of_what_is_the_meaning_of_the_estimation_error.pdf).
...
Imagine you lived $40\cdot n$ years in the future and that the population had doubled every $40$ years. For simplicity, assume that in the current present, just one person is alive. Consequently, $2^n$ people live at your point in the future. You live far in the future, and $n$ is very large: $n=10^{15}$. "Today" is your friend's birthday party, and you do not have a present.
...
In order to hire the mathgician, you have to put in your friend one petabit ($=10^{15}$ bits) long address into the website of the mathgician-firm, such that he can find the location of the party. (The address is one petabit long as, in the future, every inhabitant will have his own address, and there will be $2^n$ people.) At the party, the mathgician shows up, and everyone has a good time. But then suddenly, you start to wonder:

You only have a 1Gbit/s upload speed (hardware did not improve so much in the last $40\cdot10^{15}$ years). How have you been able to upload $10^{15}$ bits (one PentaBit) within just one day? This seems impossible as it takes $10^6$ seconds to upload one PentaBit with your network speed. Then you discover why: the website did not have to send $10^{15}$ bits of information to its server. Small parts of the information have already been known by the server; For example, the information that people in your region are more likely to order a magician for someone in the same galaxy was already present on their server. 
...
$$\mathcal L_{M,T} = \underbrace{\dfrac{\mathbb I(H_T^{(M)};\psi)}{MT}}_\text{meta estimation error} + \underbrace{\dfrac{\mathbb I(D_m;\theta_m|\psi)}{T}}_\text{intra document estimation error}$$
...
Short Answer: No, because a mixture of transformers has often an infinite context length. 
Consider a mixture with two transformers $A$ and $B$ picked with the same probability of $0.5$, that both implement an iid coinflip. 
$A$ implements the coinflip with $p=0.4$ and $B$ implements it with $p=0.7$. 
Assume we want to model this mixture with an autoregressive model with a finite horizon $m$.
Consider we generated $m+1$ coin throws. Then the heads probability of the next throw is also
dependent on a throw that is m+1 steps in the past. Therefore our model can't generate that sequence. 


In this Section we will add a construction, such that we can proof this imposibility for a class of mixtures.  
...
Notice that the sequence $X_{T_1 +1}, X_{T_2 +1}, ... | M$ is iid. To make our life easier we define the binary random variable $Y_i = \mathbf{1}[X_{T_i + 1} = y]$. Now $Y_1, Y_2...$ is an infinite sequence of coinflips, that land with probability $p_1$ with probability $p^{(1)}$ on heads and with probability $1-p_1$ with probability $p^{(2)}$ on heads.
...
Then $A(\theta)(h_t)(x) \overset{a)}{=} \mathbb P(f^{-1} (\theta)_{t+1} = x | f^{-1}(\theta)_{1:t} = h_t)$
